    ...
    def test_chat_endpoint_accepts_localhost_origin_alias(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                request = urllib.request.Request(
                    url,
                    data=json.dumps({"message": "מה מצב"}).encode("utf-8"),
                    headers={
                        "Content-Type": "application/json",
        ...
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["intent"], "HumanStatusQuestion")

    def test_chat_endpoint_rejects_nonlocal_origin(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            runtime = bootstrap(
                AppConfig(
            ...
            try:
                ...
                request = urllib.request.Request(
                    ...
                    headers={
                        "Content-Type": "application/json",
                        "Origin": f"http://example.com:{server.server_address[1]}",
                        "X-NELA-Launch-Token": "test-token",
                    },
                    method="POST",
                )
